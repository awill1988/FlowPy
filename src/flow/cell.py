...
class Cell:
    z_delta_neighbour: np.ndarray[np.float64]
    z_gamma: np.float64
    z_alpha: np.float64
    ...
    def __init__(
        ...
        # assuming this is because it's a raster?
        self.altitude = dem_ng[1, 1]

        # what does ng stand for?
        self.dem_ng = dem_ng

        # cellsize is the tile size?
        ...
        # is this precalculated? is it a threshold?
    ...
    def calc_fp_travelangle(self):
        """calculates flow path travel angle"""
        dist_min = []
    ...
    def calc_z_delta(self):
        ...
        self.z_delta_neighbour[
            self.z_delta_neighbour > self.max_z_delta
        ] = self.max_z_delta

    def calc_tanbeta(self):
        """what is the tan beta??"""
        ds = np.array(
    ...
    def calc_persistence(self):
        """calculates persistence. Also, what's persistence?"""
        self.persistence = np.zeros_like(self.dem_ng)
        if self.is_start:
            self.persistence += 1
    ...
    def calc_persistence_legacy(self):
        """old calculation of persistence. Also, what's persistence?"""
        self.persistence = np.zeros_like(self.dem_ng)
        if self.is_start:
            self.persistence += 1
        elif self.parent[0].is_start:
    ...
    def calc_distribution(self):
        """calculates the distributed flow"""
        self.calc_z_delta()
        ...
        # This lines handle if a distribution to a neighbour cell is lower then the threshold, so
        # we don't lose flux.
        # The flux of this cells will then spread equally to all neighbour cells
        count = ((self.dist > 0) & (self.dist < threshold)).sum()
        mass_to_distribute = np.sum(self.dist[self.dist < threshold])

        # Checking if flux is distributed to a field that isn't taking in account,
        # when then distribute it equally to the other fields.
